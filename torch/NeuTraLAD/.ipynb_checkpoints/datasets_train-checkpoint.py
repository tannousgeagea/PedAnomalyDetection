...
def TrainLoader(source, 
    ...
    IsNormal = [os.path.basename(content) in ['Walking', 'Running', 'Jogging', 'TalkingOnThePhone']
    ...
    if conjugate:
        datasets = np.concatenate([datasets, datasets])
    ...
    return datasets, labels, categories, np.array(index)